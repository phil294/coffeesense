...
describe('Should provide signature hints', () => {
	...
	const doc_indented_dangling_brace = getDocUri('signature-hint/indented-brace.coffee')
	const doc_indented_dangling_space = getDocUri('signature-hint/indented-space.coffee')
	...
	it('suggests params in an indented function invocation with a dangling space as trigger character', async () => {
		// This works because the space is converted into 𝆮 and thus survives the compilation
		await testSignatureHints(doc_indented_dangling_space, position(2, 39), 'signature_hint_indented_space_func(param1: any, param2: any): void')
	})
